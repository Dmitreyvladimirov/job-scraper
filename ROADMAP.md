...
- [x] SQLite как источник правды для дедупа — `load_seen_jobs()` заменил Notion lookup
- [x] `filtered_language` добавлен в таблицу `runs`; индекс на `jobs.url`
...
SQLite уже содержит все нужные данные — осталось только задеплоить.
- KPI: всего прогонов, qualified, GPT-вызовы, воронка фильтров
- Графики: qualified/fetched по дням, распределение ATS-скоров, эффективность источников, языковой фильтр
- Таблица последних прогонов с детализацией
- Защита по токену (`?token=xxx`)
- **Статус:** код написан (`dashboard.py`), деплой не завершён — нужно разобраться с Railway конфигурацией (SERVICE_TYPE=dashboard)
...
|---|---|---|
| Dashboard Railway deploy | Конфликт cron/web сервисов в одном репо, нужна отдельная конфигурация | Код готов (`dashboard.py`), деплой завершить |
