...
| Choicy (choicy.work) | **KEEP** (добавлен 2026-08-17 по запросу Дмитрия) | Bubble.io-борда с открытым Data API (`/api/1.1/obj/jobs`, без авторизации): 30 live-вакансий (Visible+Approved), полные JD, server-side фильтры/сортировка, компании резолвятся через `obj/companies`. 3/30 product-titled на момент проверки — небольшой, но чистый источник, нулевая стоимость fetch (JSON, без ScrapingBee). `sources/choicy.py` |
...
| **NULL/empty source** | **ЗАКРЫТО 2026-08-19** | Разгадано: все 54 строки — из одного прогона #8 (`sources_json: notion_import`), июньский импорт старого Notion-трекера, который не проставлял source. НЕ живая утечка — код скрапера чист. Бэкфилл: все 54 → `source=NotionImport` (URL-хосты Lever/Greenhouse/RemoteOK — это где вакансии были найдены вручную до скрапера, приписывать их скраперным источникам было бы ложью). `import_notion_csv.py` починен. Следствие: реальная атрибуция скраперных qualified всегда была полной — процентам по источникам выше можно доверять без оговорок |

## 2. Приоритизированный список действий

1. ~~**NULL-source расследование**~~ — ЗАКРЫТО 2026-08-19, см. вердикт в таблице: разовый Notion-импорт, не утечка; бэкфилл сделан, скрипт починен.
