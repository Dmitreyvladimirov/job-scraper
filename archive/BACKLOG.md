...
## [DONE] Прямая ссылка на подачу для агрегаторов (Jobicy, Jobgether) вместо ссылки на сам агрегатор

**Предложено пользователем:** 2026-07-29. **Реализовано полностью 2026-08-02 — см. TASK-001/TASK-027/TASK-028.**

**Фаза 1 (бесплатная, TASK-001/027):** механизм для этого уже был в коде (`find_apply_url()` — Greenhouse/Lever/Ashby поиск по имени компании), но был мёртвым — `_PLATFORM_HOSTS` содержал только отключённые источники. Добавлены `jobgether.com`/`jobicy.com` в гейт + почищен TASK-001 (Lever/Ashby теперь тоже проверяют имя компании). Живой тест — 35% (26/75) находят прямую ссылку бесплатно. Плюс отдельный путь для `remoteworldwide.net` (apply-href прямо в HTML).

**Фаза 2 (ScrapingBee, TASK-028):** для оставшихся — пользователь напомнил про существующий аккаунт ScrapingBee (не заводили новый Bright Data). `_fetch_via_unlocker()` в `utils.py`: Jobgether — `render_js=true` обходит Cloudflare, ссылка лежит в `data-apply-url` атрибуте отрендеренной страницы; Jobicy — сложнее, кнопка живёт в closed shadow DOM даже после JS-рендера, реальный механизм — `POST /signals.php` с одноразовым nonce (виден в inline-скрипте), в ответ JSON с настоящей ссылкой; `/signals.php` сам не за Cloudflare, ScrapingBee нужен только чтобы получить nonce с отрендеренной страницы. Живой тест: Growthspace (Jobgether→Comeet, недостижимо через фазу 1) и Reddit (Jobicy→Greenhouse, точное совпадение с примером пользователя). Без ключа — тихий no-op, ничего не ломается.

**Стоимость:** ScrapingBee считает кредиты только за `render_js`-запрос (~5 за Jobgether-вызов по заголовку `spb-cost`), не за `/signals.php`. При нашем объёме укладывается в бесплатный тариф с запасом.

**Идея:** Jobicy и Jobgether — тоже агрегаторы, не первоисточники (как и RemoteOK/Arbeitnow, для которых уже есть enrichment через `find_apply_url()` — REQ-012/013). Каждая их вакансия физически заканчивается кнопкой "Apply"/"Подаваться", которая ведёт на прямую страницу подачи в реальном ATS компании (Greenhouse/Ashby/Lever/etc.) — то есть прямая ссылка у них уже есть в разметке страницы, её не нужно угадывать по slug, как сейчас делает `find_apply_url()` для RemoteOK.

Два реальных примера с их прямыми ссылками (сохранены пользователем 2026-07-29):
- Jobgether: `https://jobgether.com/offer/6a642dd11b23f4f87b384fd0-senior-product-manager-enterprise` → `https://jobs.ashbyhq.com/Deepgram/3a12f539-7734-46fa-9e3b-28e041e56074/application?utm_source=Jobgether&utm_content=...&utm_campaign=search&utm_medium=website`
- Jobicy: `https://jobicy.com/jobs/144634-senior-technical-product-manager-ads-ml-platform` → `https://job-boards.greenhouse.io/reddit/jobs/7954001`

**Цель:** карточка в Notion должна в итоге содержать прямую ссылку на подачу (Greenhouse/Ashby/Lever/...), а не ссылку на страницу агрегатора — так же, как уже устроено для RemoteOK через `utils.py::find_apply_url()`.

**Что нужно перед реализацией (нужны инженер + продакт, не однострочный фикс):**
- Инженеру — проверить механику извлечения на реальных страницах обоих источников: лежит ли прямая ссылка в JSON-LD/структурированных данных ответа (как уже парсится для части источников), в `href` кнопки Apply в HTML, или только за JS-рендером (тогда сложнее — см. ограничение "без headless-браузера" в `requirements.md` про JS-SPA карьерные страницы). Утм-метки в URL (`utm_source=Jobgether` и т.п.) — обрезать при сохранении или оставлять как есть.
- Продакту — решить, заменяет ли прямая ссылка ссылку на агрегатор полностью (как для RemoteOK) или должны храниться обе (например, чтобы не терять контекст "откуда нашли"); как это стыкуется с дедупликацией (REQ-006) и с существующим `apply_url`/`url`/`incomplete_description` полями в `job`-словаре.
- Обоим — сколько вакансий Jobicy/Jobgether сейчас реально проходят в Notion в неделю, чтобы оценить приоритет относительно остального backlog.

**Инженерная часть investigation — сделана вживую 2026-08-02, результат хуже, чем ожидалось:**
- **Jobgether** (`jobgether.com/offer/...`): и WebFetch, и прямой запрос за Cloudflare — стабильно `403 Forbidden`, без вариантов (согласуется с уже известной заметкой в ROADMAP.md про "непредсказуемые 403" — на практике на офер-страницах это выглядит как "всегда", не "иногда"). JSON-LD-подход из ROADMAP.md (задуман для листинга/сорса вакансий) в принципе рабочий *для получения самой вакансии*, но извлечь именно прямую ссылку на подачу с офер-страницы этим способом не вышло — сама страница недоступна автоматизированному клиенту.
- **Jobicy** (`jobicy.com/jobs/...`): страница отдаёт `Content-Type: text/markdown` — это не человеческий HTML, а pre-rendered markdown-зеркало (похоже, для ботов/LLM-краулеров). Кнопка "Apply" — просто якорь `#job-application`, реальная логика подачи навешивается JS на клиенте, в разметке её нет. Есть публичный API (`jobicy.com/api/v2/remote-jobs`, задокументирован на `jobi.cy/apidocs`) — но поле `url` в ответе указывает обратно на страницу Jobicy, отдельного поля с прямой ссылкой на подачу нет. У Jobicy есть свой click-редирект (`jobicy.com/go/{id}` → `jobi.cy/{id}` → предположительно реальный таргет) — но `jobi.cy` тоже за Cloudflare и отдаёт `403` на скриптовый запрос (с разными UA/Referer/задержкой — не помогло).
- **Вывод:** для обоих источников прямую ссылку на подачу без headless-браузера с прохождением Cloudflare-челленджа (реальный JS + человекоподобное поведение) вытащить не получилось. Это прямое пересечение с уже существующим в проекте ограничением "JS-SPA карьерные страницы без Playwright — не в этом этапе" (см. `requirements.md`, "Не поддерживается"). Не однострочная доработка `find_apply_url()`, а отдельная инфраструктура (headless-браузер), которой в проекте сейчас нет и её сознательно решили не заводить.

**Приоритет:** понижен по факту investigation — ROI под вопросом, пока в проекте нет Playwright/headless-браузера ни для чего другого. Не начинать без отдельного решения, стоит ли заводить headless-браузер в принципе (это разблокировало бы и эту задачу, и JS-SPA career-page fallback из ROADMAP.md одним и тем же вложением).
...
## [TO INVESTIGATE] Источники-кандидаты (предложены пользователем 2026-07-21)

Четыре площадки, не подключённые сейчас как источники — ни одна не встречается в кодовой базе
(кроме случайного совпадения: `@remocate` как пример Telegram-канала в комментарии `config.py`,
и `remocate.app/jobs` как паттерн распознавания ссылок в `telegram_channels.py` — оба не про
`remocate.pro` конкретно и не значат, что источник реально подключён).
...
**Что нужно перед добавлением:** проверить по каждой, есть ли публичный API/RSS (как у Jobicy/
Jobgether) или только HTML-скрейпинг страницы; примерная доля PM-релевантных вакансий; гео-фокус
(есть подозрение, что часть — русскоязычные/СНГ-ориентированные площадки, как и часть уже
существующих Telegram-каналов). Формат вывода — как в `SOURCES_DECISION.md` (вердикт KEEP/FIX/DROP
+ обоснование числами), не добавлять код без этой оценки.
