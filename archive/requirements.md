...
- **REQ-124**: `enrich_url()` (`core/utils.py`) shall искать прямую ссылку на подачу для вакансий с Jobgether/Jobicy (не только для отключённых RemoteOK/Arbeitnow/WeWorkRemotely, как было раньше), через существующий `find_apply_url()` (Greenhouse/Lever/Ashby); а для агрегаторов, embed-ящих реальный apply-href прямо в HTML (`remoteworldwide.net`), shall читать его напрямую, не угадывая через ATS-slug.
  - _Acceptance_: вакансия с `url` на `jobgether.com`/`jobicy.com` получает `job["apply_url"]`, если компания найдена на Greenhouse/Lever/Ashby с подтверждённым именем (`_company_match()`); вакансия с `url` на `remoteworldwide.net` получает `job["apply_url"]` из HTML-анкора "Apply Now" без обращения к Greenhouse/Lever/Ashby.
  - _Источник_: пользователь заметил (2026-07-29/08-02) при разборе шортлиста, что большинство вакансий из Jobgether/Jobicy на самом деле ведут на Greenhouse/Ashby/Comeet — `_PLATFORM_HOSTS` содержал только отключённые источники и никогда не покрывал активные агрегаторы. Live-тест на 75 карточках — 35% (26/75) находят прямую ссылку.
- **REQ-125**: `enrich_url()` shall дополнительно пробовать headless-фолбэк через ScrapingBee (`SCRAPINGBEE_API_KEY`), когда `find_apply_url()` (REQ-124) ничего не находит для Jobgether/Jobicy — покрывает компании на ATS вне трио Greenhouse/Lever/Ashby (напр. Comeet, BambooHR).
  - _Acceptance_: вакансия с `url` на `jobgether.com`/`jobicy.com`, для которой `find_apply_url()` вернул `None`, получает `job["apply_url"]` через `_fetch_via_unlocker()`, если задан `SCRAPINGBEE_API_KEY`; без ключа — no-op, откат на ссылку агрегатора, как раньше.
  - _Источник_: пользователь напомнил про существующий аккаунт ScrapingBee 2026-08-02. Live-тест: Growthspace (Jobgether → Comeet, недостижимо через REQ-124) и Reddit (Jobicy → Greenhouse, точное совпадение с примером пользователя).
