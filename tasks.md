...
- [x] **TASK-001** [REQ-101]: добавить `_company_match()` на Lever и Ashby ветки в `utils.py::find_apply_url()`.
  ...
  - _Решение (2026-08-02)_: ни Lever, ни Ashby API не отдают имя компании напрямую — верификация через `<title>` HTML-страницы борда (`jobs.lever.co/{slug}` / `jobs.ashbyhq.com/{slug}`) + `_company_match()`. Протестировано вживую на 10 реальных вакансиях (8 Ashby + 4 Lever, часть пересекается) — 9/10 совпали с непроверенным результатом (значит ложных срабатываний не было), 1/10 (Tekion Corp) отвалился, т.к. Ashby-борд рендерится клиентским JS без company-name в `<title>`; в этом случае код консервативно возвращает `None` вместо непроверенного результата — деградации нет, просто карточка не улучшается (см. TASK-027).

- [x] **TASK-027** [REQ-124]: `_PLATFORM_HOSTS` в `utils.py` (гейт для `enrich_url()`) содержал только отключённые источники (`remoteok.com`/`arbeitnow.com`/`weworkremotely.com`) — активные агрегаторы `jobgether.com`/`jobicy.com` никогда не проходили через `find_apply_url()`, хотя код вызывался безусловно на каждой вакансии. Добавлены оба хоста в `_PLATFORM_HOSTS`.
  - _Output_: `jobgether.com`/`jobicy.com` вакансии получают `apply_url` через Greenhouse/Lever/Ashby поиск, когда компания там есть.
  - _Verify_: live-тест на 75 реальных карточках шортлиста (2026-08-02) — 35% (26/75) находят прямую ссылку без дополнительной инфраструктуры.
