...
Скоринг сейчас в shadow-режиме: локальный `core/ats.py` (GPT-4o-mini) решает,
облачный `core/scoring_client.py` пишет параллельные оценки для сравнения. Флаг
`USE_CLOUD_SCORING` (off / shadow / 1) в `core/config.py`.
...
### 1. Катовер скоринга (сейчас)

Проверить субботний отчёт облачной рутины (`trig_01HK2BhpFwuFD75aFvM2TVr5`,
отработала 2026-08-15 09:00, письмо на почте Дмитрия). Если рекомендация "go":
на сервисе "Scrapper itself" выставить `USE_CLOUD_SCORING=1` и поднять
`ATS_THRESHOLD` 60 -> 70 (`core/config.py:44`, под cloud-шкалу). Сутки
наблюдения по `ats_error` в Telegram-сводке.

Основание: на ручной разметке 25 вакансий cloud дал 80% совпадения при пороге 70
против 44% у local.

### 2. Релиз 1 резюме-фич (~6ч, ждёт формального "стартуй")

- `core/resume_client.py` - копия паттерна `scoring_client.py`, но `retries=1`:
  ретрай генерации платный.
- Колонка `jobs.resume_run_id`.
- Кнопка Generate resume в карточке (`hx-indicator`; повторный клик открывает
  уже готовое резюме).
- `GET /jobs/{id}/resume-pdf` - прямым SELECT из `resume.artifact`. База общая,
  share-ссылки с TTL не нужны.
- Гейты: `company` обязательна (иначе 422), JD короче 200 символов - блок.
- Автоскоринг ручных карточек сразу после Add-job (сейчас они не скорятся
  никогда).
- Кнопка Re-score в дубль-диалоге.

Открытый вопрос: порог автогенерации `>80` или `>=80` (рекомендация `>=80`).

### 3. Релиз 2 - автогенерация резюме (после катовера)

Батч в конце прогона скрапера. Условия: cloud-скор >=80 (порог подтвердить) AND
