...
**Current task:** Объединение JobScraper + resumebuilder-cloud. Катовер скоринга и Релиз 1 СДЕЛАНЫ 2026-08-17. Следующие шаги по порядку:
1. **Наблюдение за катовером (сутки)** — `USE_CLOUD_SCORING=1` + `ATS_THRESHOLD=70` в проде с 2026-08-17 (~15:00). Следить за `ats_error` в Telegram-сводке ближайших прогонов (cron 7 6,9,12,15 пн-пт). Откат: `USE_CLOUD_SCORING=shadow` + revert коммита порога. Отдельно: субботняя рутина shadow-отчёта (trig_01HK2BhpFwuFD75aFvM2TVr5) устарела после катовера и 15.08 всё равно падала без доступа к БД (Railway OAuth не отдаёт значения переменных) — кандидат на удаление, решение Дмитрия.
2. **Проверка Релиза 1 вживую** — код в проде (85 тестов зелёные, но по кнопкам вживую не кликали): Generate resume в peek-модалке (гейты: company, JD ≥200), Open resume + счётчик Skeptic-флагов, `GET /jobs/{id}/resume-pdf`, автоскоринг после Add-job (daemon-поток), Re-score в дубль-диалоге. На Dashboard-сервис добавлены `SCORING_TOKEN`/`RESUME_TOKEN` (Railway-референсы) — проверить первым же кликом, что резолвятся (если 401/«misconfigured» — смотреть переменные сервиса).
3. **Релиз 2 (после суток наблюдения)** — автогенерация резюме батчем в конце прогона скрапера: cloud-скор ≥80 (порог подтверждён Дмитрием 17.08) AND location_score>0 AND len(description)≥500 AND resume_run_id IS NULL, кап 5/прогон, на карточке счётчик Skeptic-флагов. Стоимость генерации $0.003 — бюджет не фактор, гейты — про качество.
...
**Last session:** 2026-08-17 (вторая сессия дня: shadow-отчёт → катовер + Релиз 1 целиком)
**Status:** 🟢 cutover live + Release 1 deployed — сутки наблюдения ats_error, затем Релиз 2
