...
## [PARTIAL] Прямая ссылка на подачу для агрегаторов (Jobicy, Jobgether) вместо ссылки на сам агрегатор

**Предложено пользователем:** 2026-07-29. **Частично реализовано 2026-08-02 — см. TASK-001/TASK-027.**

**Что сделано:** оказалось, механизм для этого уже был в коде (`find_apply_url()` — Greenhouse/Lever/Ashby поиск по имени компании), но был мёртвым — `_PLATFORM_HOSTS` содержал только отключённые источники. Добавлены `jobgether.com`/`jobicy.com` в гейт + попутно почищен TASK-001 (Lever/Ashby теперь тоже проверяют имя компании, не только Greenhouse). Живой тест на 75 карточках шортлиста — **35% (26/75) получают прямую ссылку бесплатно, без headless-браузера.** Заодно добавлен отдельный путь для `remoteworldwide.net` — там реальный apply-href лежит прямо в HTML, не за Cloudflare/JS.

**Что осталось:** оставшиеся ~65% (49/75) — компании не на Greenhouse/Lever/Ashby (похоже, Comeet/Workday/SmartRecruiters/Personio и т.п.), для них по-прежнему нужен headless-браузер + anti-bot unlocker (см. отдельное обсуждение вариантов/цены — Bright Data Web Unlocker выглядит лучшим вариантом по стоимости при нашем объёме). Отдельное решение, не блокирует уже сделанную часть.
